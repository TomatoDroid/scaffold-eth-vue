...
import { type ChainWithAttributes, getInfuraHttpUrl } from './networks'
...
export async function fetchPriceFromUniswap(targetNetwork: ChainWithAttributes): Promise<number> {
  if (
  ...
  try {
    ...
    const TOKEN = new Token(1, targetNetwork.nativeCurrencyTokenAddress
